...
- Sourcing: prefer public ATS endpoints and public GitHub repos. Never scrape Indeed or Glassdoor. LinkedIn is a deliberate, bounded exception: the `linkedin` connector reads their undocumented guest job-search endpoint, gated behind `LINKEDIN_GUEST_ENABLED` (off by default). It stays low-volume and polite — sequential requests with a delay, capped pages and capped JD fetches — and **aborts the whole run on 429/403/999**. Never add retry-into-a-block, proxy/IP rotation, or fingerprint spoofing to get around a block; if LinkedIn refuses, the connector returns what it has and the other sources carry the run.
